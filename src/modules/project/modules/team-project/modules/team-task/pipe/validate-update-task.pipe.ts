...
export class ValidateUpdateTaskPipe implements PipeTransform {
  transform(body: UpdateTeamTaskBodyDto) {
    ...
    if (body.task_time_cap) {
      const timeCapInNumber = +body.task_time_cap;
      if (isNaN(timeCapInNumber) || timeCapInNumber > 25) {
        throw new BadRequestException(
          "Task time cap should be number and can't be more than 24h",
        );
      }
      body.task_time_cap = timeCapInNumber;
    }
